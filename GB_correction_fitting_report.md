...
Part I의 GB_d²×T 모델(R²=0.94)은 두 가지 근본적 한계가 있다:
...
이로 인해 σ_proxy가 실제보다 100~1000배 과소 추정되어, R = σ_brug/σ_proxy = 15~1600으로 과장. 실제 R은 3~10.
...
## 15. Bruggeman Exponent 분해

n_eff = n_geo + n_contact = 2.54 + 0.83 = **3.37**

- n_geo = 2.54 (R²=0.71): 기하학적 tortuosity
- n_contact = 0.83 (R²=0.48): 접촉 저항 패널티. φ_SE가 48%만 설명 → 나머지 52%는 hop_area, CN, GB_d

**주의:** 이 분해는 σ_full = σ_bulk_net/R_brug에서 오는 수학적 항등식. 발견이 아님. 의미있는 것은 개별 R² 값.
...
| CN² | 2 | network connectivity | free fit: 1.98≈2 |
...
10. **CN²의 물리적 유도 미완**: Kirkpatrick EMA는 σ∝z (선형), percolation exponent t≈2는 (p-p_c)^t이지 CN^t가 아님. CN²은 empirical scaling이며, network redundancy 효과를 반영하나 엄밀한 이론적 유도는 부재.
...
1. **실제 Bruggeman overestimation은 3~10× (100~1600×가 아님)**
