...
def plot_electronic_scaling(data_list, names, outdir):
    ...
    if len(_unique) >= 5:
        ...
        if _tn.sum() >= 3:
            ...
            # √f_p × CN × por² × √cov / (φ³ × √ξ)
            _rhs_tn = SIGMA_AM * np.sqrt(_ep) * _cn[_tn] * _por_tn**2 * _cov_tn**0.5 / (_pa[_tn]**3 * _ratio[_tn]**0.5)
    ...
    for i in range(len(data_list)):
        if phi_am[i] > 0 and cn_am[i] > 0 and d_am_list[i] > 0 and thickness[i] > 0:
            ...
            if ratio_i >= 10:
                ...
                if el_perc[i] >= 0.65:
                    # √f_p × CN × por² × √cov / (φ³ × √ξ)
                    s = C_thin * SIGMA_AM * np.sqrt(el_perc[i]) * cn_am[i] * porosity[i]**2 * cov_list[i]**0.5 / (phi_am[i]**3 * ratio_i**0.5)
                else:
    ...
    txt = (f"Thick: φ⁴×CN^(3/2)×cov×√τ {tk_str}\n"
           f"Thin: √f_p×CN×por²×√cov/(φ³×√ξ) {tn_str}\n"
