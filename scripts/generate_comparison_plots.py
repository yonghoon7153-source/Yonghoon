...
PLOT_REGISTRY = {
    ...
    "rgb_fitting": {
        ...
        "description": "log(R) = α × log(GB_d²×T) + ln(C)\nR = C × (GB_d²×T)^α\n\n접촉 모델: hooke/hysteresis (Luding, 탄소성 이력)\nn=41 (SE 0.5/1.0/1.5μm, T=18~184μm, RVE 30/50μm)\n\nBLM: 입계 수 = GB_d × T\nConstriction: 입계당 저항 ∝ GB_d (점 접촉)\n→ 총 저항 ∝ GB_d² × T\n\nα = 1.82 (>1): 접촉면적 불균일성(bottleneck) + 소성변형 이력\nα=2 고정 시 R²=0.933 (ΔR²=1.0%) → robust\n\nG_path = 1/Σ(1/A_i): 직렬 접촉의 조화평균\nσ_proxy = G_path × f_perc / τ\n\nX축: log(GB_d²×T), Y축: log(σ_brug/σ_proxy)",
