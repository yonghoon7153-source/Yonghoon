...
def plot_electronic_scaling(data_list, names, outdir):
    ...
    if len(_unique) >= 5:
        ...
        if _tk.sum() >= 3:
            # Thick: CN^1.75 × (φ-φc)² / por^0.4
            _phi_ex_tk = np.clip(_pa[_tk] - 0.10, 0.001, None)
            _rhs_tk = SIGMA_AM * _cn[_tk]**1.75 * _phi_ex_tk**2 / _por[_tk]**0.4
    ...
    if len(_unique) >= 5:
        ...
        if _tk_mask.sum() >= 3:
            ...
            _pred_tk = C_thick * SIGMA_AM * np.exp(_cn_g)**1.75 * _phi_ex_g**2 / _por_g**0.4
    ...
    for i in range(len(data_list)):
        if phi_am[i] > 0 and cn_am[i] > 0 and d_am_list[i] > 0 and thickness[i] > 0:
            ...
            if ratio_i >= 8:
                ...
                s = C_thick * SIGMA_AM * cn_am[i]**1.75 * phi_ex_i**2 / porosity[i]**0.4
            else:
                # THIN: CN × δ^0.5 / √(T/d)
                if el_perc[i] >= 0.50:
    ...
    txt = (f"Thick: CN^1.75×(φ-φc)²/por^0.4 R²={r2_global_tk:.3f}(n={n_global_tk})\n"
...
def plot_electronic_decomposition(data_list, names, outdir):
    """Decompose σ_el: Thick(CN^1.75 × (φ-φc)² / por^0.4), Thin(CN × √δ / √(T/d))."""
    SIGMA_AM = 50.0
    ...
    # Thick factors: CN^1.75, (φ-φc)², por^-0.4
    phi_ex = [max(phi_am[i] - 0.10, 0.001) for i in range(n)]
    log_phi = np.array([2.0 * np.log(phi_ex[i]) for i in range(n)])
    log_cn2 = np.array([1.75 * np.log(cn_am[i]) for i in range(n)])
    log_por = np.array([-0.4 * np.log(porosity[i]) for i in range(n)])
    ...
    for i in range(n):
        if phi_am[i] > 0 and cn_am[i] > 0 and d_am[i] > 0 and thickness[i] > 0:
            if is_thick[i]:
                sigma_el.append(max(phi_am[i]-0.10,0.001)**2 * cn_am[i]**1.75 / porosity[i]**0.4)
            else:
    ...
    labels_all = ['[Thick] (φ-φc)²', '[Thick] CN^1.75', '[Thick] 1/por^0.4', '[Thin] √δ', '[Thin] 1/√(T/d)']
...
PLOT_REGISTRY["electronic_decomposition"] = {
    ...
    "description": "Thick: CN^1.75 × (φ-φc)² / por^0.4\nThin: CN × √δ / √(T/d)\n\nCN^1.75: AM-AM coordination (mono/bimo compromise)\n(φ-φc)²: percolation excess (φc=0.10)\n1/por^0.4: porosity correction\n√δ: AM-AM penetration depth\n1/√(T/d): finite-size correction",
    "origin_tip": "Stacked bar (top) + Horizontal bar (bottom).",
}
