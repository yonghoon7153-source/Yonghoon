...
def plot_electronic_scaling(data_list, names, outdir):
    ...
    if len(_unique) >= 5:
        ...
        if _tk.sum() >= 3:
            ...
            _rhs_tk = SIGMA_AM * _cn[_tk]**0.21 * _gh[_tk]**0.89 * _phi_ex_tk**2.53 / _por[_tk]**0.12 * np.exp(-0.10*_log_cn_tk*_log_gh_tk - 0.83*_log_cn_tk*_log_phi_tk - 0.33*_log_gh_tk**2)
    ...
    if len(_unique) >= 5:
        ...
        if _tk_mask.sum() >= 3:
            ...
            _pred_tk = C_thick * SIGMA_AM * np.exp(_cn_g)**0.21 * _gh_g**0.89 * _phi_ex_g**2.53 / _por_g**0.12 * np.exp(-0.10*_cn_g*_log_gh_g - 0.83*_cn_g*_log_phi_g - 0.33*_log_gh_g**2)
    ...
    for i in range(len(data_list)):
        if phi_am[i] > 0 and cn_am[i] > 0 and d_am_list[i] > 0 and thickness[i] > 0:
            ...
            if ratio_i >= 8:
                ...
                _correction = np.exp(-0.10*_log_cn_i*_log_gh_i - 0.83*_log_cn_i*_log_phi_i - 0.33*_log_gh_i**2)
                s = C_thick * SIGMA_AM * cn_am[i]**0.21 * _gh_i**0.89 * phi_ex_i**2.53 / porosity[i]**0.12 * _correction
            else:
                # THIN: CN × δ^0.5 / √(T/d)
                if el_perc[i] >= 0.50:
