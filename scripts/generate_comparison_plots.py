...
def plot_electronic_scaling(data_list, names, outdir):
    ...
    if len(_unique) >= 5:
        ...
        if _tn.sum() >= 3:
            ...
            # hop^0.3 × CN^0.3 / (φ_SE^1.5 × √ξ)
            _rhs_tn = SIGMA_AM * _hop_tn**0.3 * _cn[_tn]**0.3 / (_ps_tn**1.5 * _ratio[_tn]**0.5)
    ...
    for i in range(len(data_list)):
        if phi_am[i] > 0 and cn_am[i] > 0 and d_am_list[i] > 0 and thickness[i] > 0:
            ...
            if ratio_i >= 10:
                ...
                if el_perc[i] >= 0.65:
                    # hop^0.3 × CN^0.3 / (φ_SE^1.5 × √ξ)
                    s = C_thin * SIGMA_AM * am_hop[i]**0.3 * cn_am[i]**0.3 / (phi_se[i]**1.5 * ratio_i**0.5)
                else:
    ...
    txt = (f"Thick: φ⁴×CN^(3/2)×cov×√τ {tk_str}\n"
           f"Thin: hop^0.3×CN^0.3/(φ_SE^1.5×√ξ) {tn_str}\n"
