...
def plot_electronic_scaling(data_list, names, outdir):
    ...
    if len(_unique) >= 5:
        ...
        if _tn.sum() >= 3:
            ...
            # √f_p × por × cov / √ξ (CN 없이! 개형 우선)
            _rhs_tn = SIGMA_AM * np.sqrt(_ep) * _por_tn * _cov_tn / _ratio[_tn]**0.5
    ...
    for i in range(len(data_list)):
        if phi_am[i] > 0 and cn_am[i] > 0 and d_am_list[i] > 0 and thickness[i] > 0:
            ...
            if ratio_i >= 10:
                ...
                if el_perc[i] >= 0.65:
                    # √f_p × por × cov / √ξ
                    s = C_thin * SIGMA_AM * np.sqrt(el_perc[i]) * porosity[i] * cov_list[i] / ratio_i**0.5
                else:
