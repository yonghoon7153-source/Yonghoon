...
def plot_electronic_scaling(data_list, names, outdir):
    ...
    if len(_unique) >= 5:
        ...
        if _tn.sum() >= 3:
            ...
            _tau_tn = _tau[_tn]
            # √δ × √τ / (φ_SE × √ξ) — Holm + packing quality
            _rhs_tn = SIGMA_AM * _delta_tn**0.5 * _tau_tn**0.5 / (_ps_tn * _ratio[_tn]**0.5)
    ...
    for i in range(len(data_list)):
        if phi_am[i] > 0 and cn_am[i] > 0 and d_am_list[i] > 0 and thickness[i] > 0:
            ...
            if ratio_i >= 10:
                ...
                if el_perc[i] >= 0.65:
                    # √δ × √τ / (φ_SE × √ξ)
                    s = C_thin * SIGMA_AM * am_delta[i]**0.5 * tau[i]**0.5 / (phi_se[i] * ratio_i**0.5)
                else:
