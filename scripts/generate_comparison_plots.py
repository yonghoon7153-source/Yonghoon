...
def plot_electronic_scaling(data_list, names, outdir):
    ...
    if len(_unique) >= 5:
        ...
        if _tn.sum() >= 3:
            ...
            # hop^0.35 × CN^0.4 / (φ_SE^0.85 × √ξ)
            _rhs_tn = SIGMA_AM * _hop_tn**0.35 * _cn[_tn]**0.4 / (_ps_tn**0.85 * _ratio[_tn]**0.5)
    ...
    for i in range(len(data_list)):
        if phi_am[i] > 0 and cn_am[i] > 0 and d_am_list[i] > 0 and thickness[i] > 0:
            ...
            if ratio_i >= 10:
                ...
                if el_perc[i] >= 0.65:
                    # hop^0.35 × CN^0.4 / (φ_SE^0.85 × √ξ)
                    s = C_thin * SIGMA_AM * am_hop[i]**0.35 * cn_am[i]**0.4 / (phi_se[i]**0.85 * ratio_i**0.5)
                else:
    ...
    txt = (f"Thick: φ⁴×CN^(3/2)×cov×√τ {tk_str}\n"
           f"Thin: hop^0.35×CN^0.4/(φ_SE^0.85×√ξ) {tn_str}\n"
