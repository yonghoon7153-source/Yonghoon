...
def plot_electronic_scaling(data_list, names, outdir):
    ...
    if len(_unique) >= 5:
        ...
        if _tn.sum() >= 3:
            ...
            # por^0.7 × cov^0.2 / (√φ_SE × √ξ)
            _rhs_tn = SIGMA_AM * _por_tn**0.7 * _cov_tn**0.2 / (_ps_tn**0.5 * _ratio[_tn]**0.5)
    ...
    for i in range(len(data_list)):
        if phi_am[i] > 0 and cn_am[i] > 0 and d_am_list[i] > 0 and thickness[i] > 0:
            ...
            if ratio_i >= 10:
                ...
                if el_perc[i] >= 0.65:
                    # por^0.7 × cov^0.2 / (√φ_SE × √ξ)
                    s = C_thin * SIGMA_AM * porosity[i]**0.7 * cov_list[i]**0.2 / (phi_se[i]**0.5 * ratio_i**0.5)
                else:
    ...
    txt = (f"Thick: φ⁴×CN^(3/2)×cov×√τ {tk_str}\n"
           f"Thin: por^0.7×cov^0.2/(√φ_SE×√ξ) {tn_str}\n"
